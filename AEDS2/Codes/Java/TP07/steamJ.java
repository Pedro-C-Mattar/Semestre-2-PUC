...
class NoAN {
    ...
    // constructor default: BLACK (seguindo a lógica original que criava nós inicialmente com cor false)
...
class ArvoreAlvinegra {
    ...
    public void inserir(Game x) {
        raiz = inserir(x, raiz);
        if (raiz != null) raiz.cor = false; // raiz sempre BLACK
    }
    ...
    private NoAN inserir(Game x, NoAN i) {
        if (i == null) {
            ...
            // caso com dois filhos: chama a versão com bisavo/avo/pai para inserir em nível mais profundo e balancear
    ...
    private void balancear(NoAN bisavo, NoAN avo, NoAN pai, NoAN i) {
        if (pai.cor == true) {
            ...
            if (pai.elemento.getName().compareTo(avo.elemento.getName()) > 0) { // pai é à direita do avô -> dir dir ou dir esq
...
class ArvoreDeArvore {
    ...
    private No1 buscarNo1(int chave, No1 i) {
        ...
        if (chave < i.chave) return buscarNo1(chave, i.esq);
        return buscarNo1(chave, i.dir);
    }

    // inserir um Game: calcula chave e insere o nome na árvore secundária correspondente
    ...
    private No2 inserirSegunda(String name, No2 i) {
        if (i == null) return new No2(name);
        if (name.compareTo(i.name) < 0) i.esq = inserirSegunda(name, i.esq);
        else if (name.compareTo(i.name) > 0) i.dir = inserirSegunda(name, i.dir);
        return i;
    }
    ...
    public boolean pesquisarComCaminho(String nome) {
        ...
        if (!found) System.out.println("NAO");
        return found;
    }
    ...
    private boolean pesquisarPrimeiraComCaminho(String nome, No1 i) {
        ...
        if (i.esq != null) {
            System.out.print("ESQ ");
            if (pesquisarPrimeiraComCaminho(nome, i.esq)) return true;
        }
        if (i.dir != null) {
            System.out.print("DIR ");
            if (pesquisarPrimeiraComCaminho(nome, i.dir)) return true;
        }
...
public class steamJ {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (!line.equals("FIM")) {
            int pos = findID(line);
            if (pos != -1) {
                arvore.inserirGame(fullDB[pos].clone());
            }
            line = sc.nextLine();
        }
        ...
        while (!nome.equals("FIM")) {
            if (nome.length() != 0) {
                arvore.pesquisarComCaminho(nome);
            }
            nome = sc.nextLine();    
        }
        ...
        // Arquivo de log
        PrintWriter log = new PrintWriter("888302_arvoreDeArvore.txt");
        log.println("888302 " + tempo + " " + arvore.comparacoes);
        log.close();
    }
}